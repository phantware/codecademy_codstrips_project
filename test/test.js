...
describe("Express server", function () {
  it("is instantiated and exported from **app.js**", function () {
    ...
    expect(
      app.unlink,
      "Did you export an express application with `module.exports`?"
    ).to.deep.equal(myApp.unlink);
  });
  ...
  it("listens at the correct process.env.PORT or 4001.", function () {
    ...
    assert.isOk(
      isMatch,
      "Did you use `app.listen` to start your server listening at the correct port?"
    );
  });
});
...
describe("Strip table", function () {
  it("should exist", function (done) {
    prodDb.get(
      "SELECT name FROM sqlite_master WHERE type='table' AND name='Strip'",
      (error, table) => {
        if (error || !table) {
  ...
  it("should have head, body, bubble_type, background, bubble_text, and caption columns with appropriate data types", function (done) {
    prodDb.run(
      "INSERT INTO Strip (head, body, bubble_type, background, bubble_text, caption) VALUES ('angry', 'plus', 'statement', 'boat', 'test text', 'test caption')",
      function (error) {
        if (error) {
          done(error);
        } else {
          prodDb.run(
            `DELETE FROM Strip WHERE Strip.id = ${this.lastID}`,
            () => {
  ...
  it("should have a required head column", function (done) {
    prodDb.run(
      "INSERT INTO Strip (body, bubble_type, background, bubble_text, caption) VALUES ('plus', 'statement', 'boat', 'test text', 'test caption')",
      function (error) {
  ...
  it("should have a required body column", function (done) {
    prodDb.run(
      "INSERT INTO Strip (head, bubble_type, background, bubble_text, caption) VALUES ('angry', 'statement', 'boat', 'test text', 'test caption')",
      function (error) {
        if (error && error.toString().includes("NOT NULL constraint failed")) {
          done();
        } else if (!error) {
  ...
  it("should have a required background column", function (done) {
    prodDb.run(
      "INSERT INTO Strip (head, body, bubble_type, bubble_text, caption) VALUES ('angry', 'plus', 'statement', 'test text', 'test caption')",
      function (error) {
        if (error && error.toString().includes("NOT NULL constraint failed")) {
          done();
        } else if (!error) {
  ...
  it("should have a required bubble_type column", function (done) {
    prodDb.run(
      "INSERT INTO Strip (head, body, background, bubble_text, caption) VALUES ('angry', 'plus', 'boat', 'test text', 'test caption')",
      function (error) {
        if (error && error.toString().includes("NOT NULL constraint failed")) {
          done();
        } else if (!error) {
  ...
  it("should have a required bubble_text column", function (done) {
    prodDb.run(
      "INSERT INTO Strip (head, body, background, bubble_text, bubble_type, caption) VALUES ('angry', 'plus', 'boat', null, 'question', 'test caption')",
      function (error) {
        if (error && error.toString().includes("NOT NULL constraint failed")) {
          done();
        } else if (!error) {
  ...
  it("should default bubble_text column to an empty string", function (done) {
    prodDb.run(
      "INSERT INTO Strip (head, body, background, bubble_type, caption) VALUES ('angry', 'plus', 'boat', 'question', 'test caption')",
      function (error) {
        if (error && error.toString().includes("NOT NULL constraint failed")) {
          done();
        } else if (!error) {
          prodDb.get(
            `SELECT * FROM Strip WHERE Strip.id = ${this.lastID}`,
            (err, row) => {
              if (err) {
                throw err;
              } else {
  ...
  it("should default caption column to an empty string", function (done) {
    prodDb.run(
      "INSERT INTO Strip (head, body, background, bubble_type, bubble_text) VALUES ('angry', 'plus', 'boat', 'question', 'test text')",
      function (error) {
        if (error && error.toString().includes("NOT NULL constraint failed")) {
          done();
        } else if (!error) {
...
describe("POST /strips route", function () {
    ...
    request(app)
      ...
      .then(function () {
        testDb.all("SELECT * FROM Strip", function (error, result) {
          const strip = result.find(
            (strip) => strip.caption === validStrip.caption
          );
  ...
  it("should return a 201 status code after strip creation", function () {
    return request(app).post("/strips").send({ strip: validStrip }).expect(201);
  });
