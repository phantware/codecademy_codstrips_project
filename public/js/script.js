...
const awsPrefix =
  "https://s3.amazonaws.com/codecademy-content/programs/build-apis/projects/build-apis-mini-capstone-project/";
...
const populateStrips = () => {
  strips.forEach((strip) => {
    $("ul").append(
      `<li id="strip-button-${strip.id}" class="button strip-button">${strip.caption}</li>`
    );
  });
};
...
const addStrip = (newStrip) => {
  $("ul").append(
    `<li id="strip-button-${newStrip.id}" class="button strip-button">${newStrip.caption}</li>`
  );
  strips.push({
    head: newStrip.head,
    body: newStrip.body,
...
const drawStrip = () => {
  $("#head").attr(
    "src",
    `${findInArrayByName(options.head, current.head).src}`
  );
  $("#body").attr(
    "src",
    `${findInArrayByName(options.body, current.body).src}`
  );
  $("#bubble").attr(
    "src",
    `${findInArrayByName(options.bubble, current.bubble).src}`
  );
  $("#background").attr(
    "src",
    `${findInArrayByName(options.background, current.background).src}`
  );
  $(".display-container").css(
    "background-color",
    `${findInArrayByName(options.background, current.background).bgColor}`
  );
  redrawText();
};
